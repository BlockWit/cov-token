...
describe('ERC20', function () {
  ...
  beforeEach(async function () {
    ...
    await this.token.adminMint(initialHolder, initialSupply)
  });
  ...
  describe('_mint', function () {
    ...
    it('rejects a null account', async function () {
      await expectRevert(
        this.token.adminMint(ZERO_ADDRESS, amount), 'ERC20: mint to the zero address',
      );
    });

    describe('for a non zero account', function () {
      beforeEach('minting', async function () {
        const { logs } = await this.token.adminMint(recipient, amount);
        this.logs = logs;
      });
  ...
  describe('_burn', function () {
    it('rejects a null account', async function () {
      await expectRevert(this.token.adminBurn(ZERO_ADDRESS, new BN(1)),
        'ERC20: burn from the zero address');
    });

    describe('for a non zero account', function () {
      it('rejects burning more than balance', async function () {
        await expectRevert(this.token.adminBurn(
      ...
      const describeBurn = function (description, amount) {
        describe(description, function () {
          beforeEach('burning', async function () {
            const { logs } = await this.token.adminBurn(initialHolder, amount);
            this.logs = logs;
          });
